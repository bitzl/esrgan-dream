    ...
    def blurry_noise(self):
        ...
        match self.color_mode:
            case ColorMode.color:
                ...
                img = rng.integers(max_value, size=(width, height, 3), dtype=np.uint8) + self.color_offset
                if self.blur > 0:
                    img = cv2.blur(img, (self.blur, self.blur))
            case ColorMode.grayscale:
                print("Generate grayscale image")
                img = rng.integers(max_value, size=(width, height, 1), dtype=np.uint8) + self.color_offset
                if self.blur > 0:
                    img = cv2.blur(img, (self.blur, self.blur))
            case ColorMode.black_and_white:
                print("Generate black and white image")
                img = rng.integers(255, size=(width, height, 1), dtype=np.uint8)
                img[img < 128] = 0
                img[img >= 128] = 255
        return img
